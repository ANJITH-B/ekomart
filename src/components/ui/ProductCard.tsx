...
export default function ProductCard({ product }: ProductCardProps) {
  ...
  return (
    <motion.div
      className="bg-white rounded-2xl border border-gray-300 hover:border-[#629D23]/50   transition-all duration-300 group overflow-hidden product-card relative"
    >
      <Link href={`/shop/${product.slug}`}>
        {/* Image Container */}
        <div className="relative bg-white overflow-hidden" style={{ paddingBottom: '80%' }}>
          <Image
            src={product.image}
            alt={product.name}
            fill
            className="p-10 object-contain transition-transform duration-500 group-hover:scale-105 "
          />
